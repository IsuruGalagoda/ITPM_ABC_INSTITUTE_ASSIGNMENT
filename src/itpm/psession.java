...
public class psession {
     public void insertUpdateDeleteStudent(char operation, int id, String Lec, String Tag, String Year, String Group, String Subject, String NoStudents, String Duration ){
        ...
        PreparedStatement ps;
         ...
         if(operation == 'u'){
            try {
                ps = con.prepareStatement("UPDATE `psession` SET  `Lec`= ?,`Tag`= ?,`Year`= ?,`Group`= ?,`Subject`= ?,`NoStudents`= ?,`Duration`= ? WHERE `id`= ?");
         ...
         if(operation == 'i'){
            try {
                ps = con.prepareStatement("INSERT INTO psession(id, Lec, Tag, Year, Group, Subject, NoStudents,  Duration) VALUES (?,?,?,?,?,?,?,?)");
                ps.setInt(1, id);
                ps.setString(2, Lec);
                ps.setString(3, Tag);
                ps.setString(4, Year);
         ...
         if(operation == 'd'){
            try {
                ps = con.prepareStatement("DELETE FROM `psession` WHERE `id` = ?");
    ...
    public void fillStudentJtable(JTable table, String valueToSearch){
        ...
        PreparedStatement ps;
        try {
            ps = con.prepareStatement("SELECT * FROM `psession` WHERE concat (`ID`,`Lec`,`Tag`,`Year`,`Group`,`Subject`,`NoStudents`,`Duration`) LIKE ?");
